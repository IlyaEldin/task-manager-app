...
export function useFormTask() {
    ...
    const changePointArray = (point, value) => {
        ...
        setFormDataCache((prev) => ({ ...prev, [point]: [...prev[point], value] }))
    }

    const changeForm = (point, value) => {

        const textPoint = [FORM_KEY.title, FORM_KEY.description, FORM_KEY.priority, FORM_KEY.dueDate];
        const arrayPoint = [FORM_KEY.tags, FORM_KEY.subtasks];

        if (textPoint.includes(point)) { changePointText(point, value); return; }
        if (arrayPoint.includes(point)) { changePointArray(point, value); return; }
